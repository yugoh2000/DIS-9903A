...
# Step 7: Perfoming Hyperparameter Tuning with Optuna (Optional Experiment)
...
# Step 9: Predicting 2024 Enrollment Values and Comparing them with the Actual Enrollment Data
...
predictions_df.to_csv('~/Documents/PhD/PhD-DS/Dissertation Files/Predicted_Enrollment_2024_with_Accuracy_Using_2015-2023_Data_2.csv', index=False)
...
# Step 10: Performing Hypothesis Testing (T-Test) on Residuals for Statistical Significance
...
# Calculate residuals (difference between actual and predicted values) for each model
predictions_df['RF Residuals'] = predictions_df['Actual Enrollment 2024'] - predictions_df['Random Forest']
predictions_df['LR Residuals'] = predictions_df['Actual Enrollment 2024'] - predictions_df['Linear Regression']
predictions_df['DT Residuals'] = predictions_df['Actual Enrollment 2024'] - predictions_df['Decision Tree']
predictions_df['NN Residuals'] = predictions_df['Actual Enrollment 2024'] - predictions_df['Neural Network']
predictions_df['XGB Residuals'] = predictions_df['Actual Enrollment 2024'] - predictions_df['XGBoost']
predictions_df['Stacked Residuals'] = predictions_df['Actual Enrollment 2024'] - predictions_df['Stacked Model']

# Calculate p-values for each model using a paired t-test
p_values = {
    'Random Forest': stats.ttest_1samp(predictions_df['RF Residuals'], 0).pvalue,
    'Linear Regression': stats.ttest_1samp(predictions_df['LR Residuals'], 0).pvalue,
    'Decision Tree': stats.ttest_1samp(predictions_df['DT Residuals'], 0).pvalue,
    'Neural Network': stats.ttest_1samp(predictions_df['NN Residuals'], 0).pvalue,
    'XGBoost': stats.ttest_1samp(predictions_df['XGB Residuals'], 0).pvalue,
    'Stacked Model': stats.ttest_1samp(predictions_df['Stacked Residuals'], 0).pvalue
}

# Convert p-values dictionary to DataFrame for visualization
p_values_df = pd.DataFrame(list(p_values.items()), columns=['Model', 'P-Value'])

# Display the p-values DataFrame
print(p_values_df)

# Optional: Plot the p-values for each model
plt.figure(figsize=(10, 6))
sns.barplot(x='P-Value', y='Model', data=p_values_df)
plt.title('P-Values for Each Model Prediction vs. Actual Enrollment')
plt.xlabel('P-Value')
plt.ylabel('Model')
